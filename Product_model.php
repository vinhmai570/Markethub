...
class Product_model extends CI_Model {
    ...
    public function getSaleProduct()
    {
        ...
        $this->db->where('discount >= 40');
        $this->db->where('DATEDIFF(CURDATE(), product.update_date) <= 1');
        $SaleProduct = $this->db->get();
        if($SaleProduct->num_rows() > 0)
        {
            return $SaleProduct->result_array();
        }else { return 0; }
    }
